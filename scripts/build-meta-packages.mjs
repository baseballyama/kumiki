...
async function writeFormatted(filePath, content, parser) {
  const config = await prettier.resolveConfig(filePath);
  const formatted = await prettier.format(content, { ...config, parser, filepath: filePath });
...
async function generateMetaPackage({ name, description, slugDir, status, entries }) {
  ...
  await writeFormatted(path.join(dir, 'package.json'), JSON.stringify(pkg), 'json-stringify');
...
`;
  await writeFormatted(path.join(dir, 'README.md'), readme, 'markdown');
  ...
  await writeFormatted(path.join(dir, 'src/index.ts'), indexBody, 'typescript');
  ...
  for (const e of entries) {
    ...
    await writeFormatted(path.join(dir, 'src', `${e.slug}.ts`), body, 'typescript');
  }
}
