...
>
  ...
  <h2>계층별 비상구</h2>
  <p>
    <code>child</code> 로도 부족하다면 — 예컨대 세 개의 커스텀 DOM 노드를 조정해야 한다면 — 한 계층 내려가세요.
...
<button {@attach t.root} class="my-fancy-toggle">
  ...
  <h2>제네릭이 전파됨</h2>
  <p>
    타입이 지정된 값을 받는 컴포넌트(Combobox, RadioGroup, Select, FormField)는 그 제네릭을 Root 에서
    전파합니다. 내부 서브컴포넌트는 <code>getContext</code> 로 읽으므로, 각 계층에서 타입을 반복할
    필요가 없습니다.
  </p>
</Prose>
