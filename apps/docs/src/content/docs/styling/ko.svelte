<script lang="ts">
  ...
  const stack = [
    {
      layer: '제품 베이스라인',
      tool: '전역 CSS [data-component*] 셀렉터',
      use: '모든 Dialog 가 공유하는 리셋 스타일',
    },
    {
      layer: '디자인 시스템 부품',
      tool: '서브컴포넌트로 class 패스스루',
      use: '<MyDialog> 의 구조 스타일링',
    },
    { layer: '변형 / 테마', tool: 'CSS 커스텀 프로퍼티', use: '브랜드 컬러, 다크 / 라이트 전환' },
    {
      layer: '상태 차이',
      tool: 'data-state 셀렉터(또는 Tailwind data-[state=open]:)',
      use: '열림 / 닫힘, 선택, 비활성, 호버',
    },
...
>
  ...
  <p>
    Layer 4 는 <strong>의미 있는 DOM + ARIA + <code>data-*</code> 속성</strong>만 내보냅니다. 의도된
    결정입니다:
  </p>
  <ul>
    <li>
      <strong>번들 예산</strong>: Toggle 1.5 KB / Dialog 3.5 KB / Combobox 4.5 KB — CSS 가 들어갈
      자리가 없습니다.
    </li>
    <li>
      <strong>디자인 시스템은 제각각</strong>: Tailwind / UnoCSS / 바닐라 CSS / CSS-in-JS — 아무것도
      결정하지 않음으로써 모두에 맞춰집니다.
    </li>
    <li>
      <strong>애니메이션도 CSS 주도</strong>: 우리는 <code>data-state="open|closed"</code> 만 내보냅니다.
      CSS Transitions, View Transitions, 모션 라이브러리 중에서 당신이 고릅니다.
    </li>
  </ul>
...
[data-state='closed'] { animation: fade-out 150ms; }`}</code
  ...
  <p>
    Layer 4 서브컴포넌트는 <strong
      ><code>...rest</code> 를 spread 하는 얇은 단일-엘리먼트 래퍼</strong
    >입니다. 당신이 전달하는 <code>class</code>, <code>style</code>, 추가 <code>data-*</code>, 추가
    ARIA 속성은 모두 실제 DOM 루트에 떨어집니다.
  </p>
  ...
  <p>
    구현 참고: <code>packages/components/src/toggle/Root.svelte</code> 는 Props 타입에
    <code>[key: string]: unknown</code>
    을 선언하고 <code>...rest</code> 를 자체 <code>&lt;button&gt;</code> 에 직접 spread 합니다.
  </p>

  <h2>레시피 3: CSS 커스텀 프로퍼티(테마 전파의 표준 경로)</h2>
  <p>
    Svelte 의 scoped CSS 와 달리 <strong>CSS 변수는 정상 캐스케이드를 따릅니다</strong>. 부모에서
    선언하면 자식 컴포넌트 내부 DOM 까지 도달하며 — Svelte 의 스코프 장벽을 완전히 우회합니다.
  </p>
...
<style>
  ...
  <p>
    기본적으로 <code>Toggle.Root</code> 는 <code>&lt;button&gt;</code> 을 렌더합니다. "여기는
    <code>&lt;a&gt;</code>
    가 필요해" 또는 "내 <code>&lt;MyButton&gt;</code> 을 쓰고 싶어" 를 위한 비상구입니다.
  </p>
  ...
  <p>
    <code>props</code> 는 완전히 타입화됩니다: <code>type</code> / <code>aria-pressed</code> /
    <code>aria-disabled</code>
    / <code>data-state</code> / <code>onclick</code> / <code>onkeydown</code> / <code>id</code>.
    그것을 당신의 엘리먼트에 spread 하는 것이 당신의 일입니다.
  </p>

  <p class="note">
    <strong>기본으로 이걸 손에 잡지 마세요.</strong> <code>child</code> 는 비상구이지 표준 스타일링
    경로가 아닙니다. <code>class</code> 패스스루로 충분하다면 그것을 선호하세요 — 그리고
    <code>props</code> 를 다시 spread 하는 것은 당신 책임이라는 것을 기억하세요(잊으면 ARIA / 이벤트가
    빠집니다).
  </p>
...
" />`}</code
  ...
  <h3>UnoCSS(기본 모드)</h3>
  <p>
    Tailwind 와 동일한 작성 경험. <code>data-[state=on]:</code> 변형은
    <code>@unocss/preset-mini</code>
    / <code>preset-wind3</code> 에 내장되어 있습니다.
  </p>

  <h3>UnoCSS svelte-scoped 모드</h3>
  <p>
    <code>@unocss/svelte-scoped</code> 는 각 부모 <code>.svelte</code> 를 스캔한 다음 생성된 CSS 를
    <strong><code>:global(...)</code> 로 감싸서</strong>
    그 파일의 <code>&lt;style&gt;</code> 에 주입합니다. 규칙이 전역이므로 부모에서 작성한 utility 가 추가
    작업 없이 자식 컴포넌트 내부 DOM 까지 도달합니다.
  </p>
...
<MyToggle bind:pressed={muted} aria-label="음소거">
  ...
  <p>
    오래된 Svelte 제약: 부모 <code>.svelte</code> 의 <code>&lt;style&gt;</code> 에 정의된 클래스는
    <strong>자식 컴포넌트 내부의 DOM 엘리먼트에 도달하지 않습니다</strong>.
  </p>
...
<style>
  ...
  <ol>
    <li>
      <strong>Tailwind / UnoCSS / 전역 스타일시트 사용</strong>(레시피 1, 5). 애초에 scoped 가
      아니므로 문제가 없습니다. <strong>1순위 권장.</strong>
    </li>
    <li>
      <strong>각 서브컴포넌트에 <code>class</code> 전달</strong>(레시피 2).
      <code>&lt;Combobox.Input class="ds-input" /&gt;</code>
      은 자식의 루트 엘리먼트에 클래스를 떨어뜨립니다. 부모 <code>&lt;style&gt;</code> 안에서는
      <code>:global(.ds-input)</code>
      으로 쓰거나 <code>app.css</code> 로 옮기세요.
    </li>
    <li>
      <strong>CSS 커스텀 프로퍼티</strong>(레시피 3). Svelte 스코프를 지나 캐스케이드합니다. 테마
      전파에 가장 적합.
    </li>
    <li>
