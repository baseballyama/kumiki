...
<LocaleProvider.Root locale={active} messages={bundle.messages} dir={bundle.direction}>
  ...
  <p>
    ...
    للاتّجاه (Tabs <code>ArrowRight</code> وSlider وRadioGroup) الاتّجاه من سياق الآلة — لا يعرف
    الـ controller شيئًا عن RTL.
  </p>

  <p>
    يُتيح <strong>زر تبديل الاتّجاه</strong> في كل صفحة تفاصيل مكوّن معاينة RTL لأيّ locale دون
    تبديل اللغة. استخدمه لاختبار التنسيق.
  </p>
