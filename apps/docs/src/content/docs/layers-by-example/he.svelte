...
>
  <p>
    ...
    משאירה בידיך עוד DOM, ARIA והאזנה לאירועים — אבל גם מעניקה חופש לבחור את המבנה. עלייה בשכבה
    אחת — פחות קוד, אבל אתה מקבל את הבחירות המבניות של Kumiki.
  </p>

  <p>
    אנו משתמשים ב-<a href="/components/component-toggle">Toggle</a> כדוגמה מובנית. ההתנהגות פשוטה
    (לחץ כדי להחליף), אבל היישום קיים ב<strong>כל ארבע השכבות</strong>, ולכן אידיאלי להשוואה זו לצד זו.
  </p>
...
<Toggle.Root bind:pressed aria-label="השתק שמע">
  ...
  <p>
    <strong>בחר זאת כאשר:</strong> 90% מהמקרים. ה-<code>&lt;button&gt;</code> הטבעי מספיק, ואינך
    צריך לעקוף את מבנה העטיפה.
  </p>

  <h2>2. Layer 3 — Headless attachment</h2>
  <p>
    כשצריך לבחור את האלמנט בעצמך (<code>&lt;a&gt;</code>, <code>&lt;div role="button"&gt;</code>,
    עטיפה מותאמת). <code>createToggle()</code> מחזירה מפעל תואם ל-<code>{`{@attach}`}</code> שניתן
    לפזר על כל אלמנט שתבחר.
  </p>
...
<button {@attach t.root} aria-label="השתק שמע" class="my-btn">
  ...
  <p>
    ...
    (Space / Enter). <strong>מה שמתווסף לאחריותך:</strong> בחירת האלמנט, העיצוב, התווית הגלויה,
    ו(רק אם נחוץ) <code>subscribe</code> כדי לשקף סטייט למשתנים ראקטיביים.
  </p>
  ...
  <p>
    בלי Svelte בכלל — מכונת מצב סופית ב-TypeScript טהור. <strong>אתה כותב את ה-DOM, ARIA, האירועים
    והמקלדת בעצמך.</strong>
  </p>
...
<Toggle bind:pressed>השתק</Toggle>`}</code
  ...
  <p>
    ...
    <code>0.x.x-preview</code> במהלך סדרת v1.0, כך שלפרויקטים רגישים ליציבות העדף Layer 4 + עיצוב
    משלך.
  </p>
