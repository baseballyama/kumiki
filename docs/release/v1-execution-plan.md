...
- [x] **bundle 列 (L4)**: `.svelte` の brotli 計測手段を確立
  ...
  - **継続課題（v1.0 後 ADR + PR）**:
    - `@kumiki/components/datetime-field` — DatePart/TimePart を別 subpath に切り出して `exports` 形状変更（要 maintainer 承認）→ **ADR 0019 (Proposed) 起票済み 2026-05-10**
    - `@kumiki/components/button`, `@kumiki/components/toggle` — L3 `paint()` ↔ L4 reactive bindings の重複削減（L3 contract change）→ **ADR 0020 (Proposed) 起票済み 2026-05-10**
    - `@kumiki/components/form-field` — `with-validation` subpath 分離 → **ADR 0021 (Proposed) 起票済み 2026-05-10**
    - `@kumiki/components/accordion` — Item context 簡素化（ADR 未起票、accordion の Item context は機能要件が大きく v1.0 後の独立検討が必要）
    - `@kumiki/components/alert`（残差）— locale-provider inlining（ADR 未起票、locale-provider の inlining は他コンポーネントへ波及するため横断 ADR が必要）
...
- [x] 全 L4 / Atelier コンポーネントが SvelteKit SSR で落ちずに HTML 出力する
  - 受入: `apps/docs` の SSR が JS 無効状態で全 sandbox を 200 + 構造的マーカー付きで返却
  - 見積: S → **完了**
  - 依存: なし
  - **完了 2026-05-10**: 当初想定していた "全 doc route に `prerender = true` を打つ" 路線は、`/sizes` で既知の favicon-missing crash や、sandbox 群が `page.url.searchParams` 由来の状態を持つことから採用せず（hydration mismatch リスクが高い）。代わりに **Playwright `ssr` project (javaScriptEnabled: false)** を新設し、`apps/docs/tests/sandbox.ssr.test.ts` で全 41 sandbox route を JS 無効で訪問、HTTP 200 + per-component anchor (data-testid / data-component) が SSR'd HTML に存在することを検証する形に切替。これは prerender よりも厳しい — vite dev の SSR を実 request で叩くため、SvelteKit lifecycle・layout・load・dot-namespace barrel すべてを通った "本番と同じ"レンダリングパスを通す。`pnpm --filter @kumiki/docs test:ssr` で 41/41 green を確認。Atelier (atelier-dialog / atelier-toggle / atelier-gallery) と Layer 3 bare-metal fixture (toggle-bare) も含む。これにより Layer 4 / Atelier component のいずれかが `document.` 等を top-level で読むような regression は CI で必ず block。
...
## 着手順（次の 2 週間） — 2026-05-10 更新（A-3 #2 SSR / A-3 #1 / FSM JSON visible / Deliverable F 完了後）

> A-3 #2 (全 41 sandbox の SSR スモーク, Playwright `ssr` project, JS 無効で 41/41 green) も追加クローズ。これで A-3 トラックは完了。残るは **C-5 visual regression**、A-2 a11y 列、D-2 残作業 (Headless UI + 競合再測定)、ADR 0019/0020/0021 maintainer review、E-1 共同メンテナ。
...
4. **ADR 0019/0020/0021 maintainer review → Accepted 化** — Proposed のままだと実装着手不可。merge 後は v1.0 後の実装 PR（exports 変更を伴うため要慎重 review）
