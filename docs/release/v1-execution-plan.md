...
## 着手順（次の 2 週間） — 2026-05-10 更新（C-2 完了後）

> C-2 atelier shape 統一 (folder shape 全件統一 + script ハード規約化) が完了。残る Atelier GA 周辺は C-5 visual regression のみ。
...
3. **D-2 公開計測ページ着手** — `apps/docs/sizes/` に bundle 比較表（kumiki vs Bits/Melt/Headless）の data ingestion を実装。A-2 measurement infrastructure の output を直接食わせる
