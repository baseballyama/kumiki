...
## 着手順（次の 2 週間） — 2026-05-10 更新（A-3 #1 / FSM JSON visible / Deliverable F 完了後）

> A-3 #1 (Node 22 import + L3 controller construct, both DOM-globals-free) と FSM JSON spec の playground 露出 (`/play/[package]` の State machine セクション) を追加クローズ。Deliverable F (per-subpath bundle budget) は A-2 で実質達成済みのためチェック化。Atelier GA 残りは **C-5 visual regression**、D-2 残作業は **Headless UI 追加 + 競合自動再測定パイプライン**、A-2 a11y 列、A-3 #2 SSR 検証、ADR 0019/0020/0021 maintainer review。
...
3. **A-3 #2 SSR 検証** — sandbox 系 route に `prerender = true` を付け、`apps/docs` build で全 L4 component が HTML 出力されることを確認、failure があれば block list 化（S）
4. **D-2 残課題** — `/sizes/compare` に Headless UI 行を追加、`scripts/refresh-competitor-sizes.mjs` を実装して競合数値を実測ベースに昇格。`generatedAt` を CI で自動更新
5. **ADR 0019/0020/0021 maintainer review → Accepted 化** — Proposed のままだと実装着手不可。merge 後は v1.0 後の実装 PR（exports 変更を伴うため要慎重 review）
